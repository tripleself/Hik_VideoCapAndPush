...
public:
	BYTETracker(int frame_rate = 30, int track_buffer = 30,
				...
				float unconfirmed_thresh = 0.7f, float low_match_thresh = 0.5f);
	~BYTETracker();
...
private:
	...
	float high_thresh;
	float match_thresh;
	float unconfirmed_thresh; // Threshold for unconfirmed tracks matching
	float low_match_thresh;	  // Threshold for low score detections matching
	int frame_id;
	int max_time_lost;
